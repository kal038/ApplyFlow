...
export function JobCard({
  ...
  return (
    ...
    >
      {/* top accent bar */}
      <div className="absolute inset-x-0 top-0 h-0.5 rounded-t-xl bg-gradient-to-r from-primary/60 via-primary/30 to-primary/60 opacity-0 transition-opacity group-hover:opacity-100" />
      ...
      <div className="flex items-start gap-3 p-4 pb-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 space-y-1">
              <h3 className="truncate text-sm font-semibold leading-tight">{job.company}</h3>
              <p className="truncate text-[13px] text-muted-foreground">{job.title}</p>
            </div>
        ...
        <div className="absolute right-2 top-2 flex items-center gap-1 opacity-0 transition-opacity focus-within:opacity-100 group-hover:opacity-100">
          ...
          <Button
            ...
            disabled={isDemo}
            className="h-7 w-7 text-muted-foreground hover:bg-red-500/10 hover:text-red-600 dark:hover:bg-red-500/20"
          >
      ...
      <div className="mt-auto flex items-center justify-between rounded-b-xl border-t bg-muted/20 px-4 py-2">
        <span className="text-[11px] text-muted-foreground">ID: {job.job_id.slice(0, 8)}</span>
